...
CHORD_PROGRESSIONS = {
    "HAPPY":  [[0,4,7,11],[5,9,12,16],[7,11,14,17],[0,4,7,11]],
    "SAD":    [[0,3,7,10],[5,8,12,15],[7,10,14,17],[0,3,7,10]],
    "ANGRY":  [[0,3,6,9],[5,8,11,14],[10,13,17,20],[0,3,6,9]],
    "CALM":   [[0,4,7,9],[2,5,9,12],[4,7,11,14],[0,4,7,9]],
    "UNCERTAIN":[[0,3,7,10],[5,8,12,15],[7,10,14,17],[0,3,7,10]],
}
...
EMOTION_PARAMS = {
    ...
    "ANGRY":    {"mode":"harmonic_minor",  "leap_prob":0.55, "rest_prob":0.02, "direction": 0.0,  "tension":0.7},
    "CALM":     {"mode":"lydian", "leap_prob":0.08, "rest_prob":0.22, "direction": 0.15, "tension":0.05},
...
def _compose_melody_voice(scale: list, length: int, params: dict, start_idx: int = None) -> list:
    ...
    leap_prob = params.get("leap_prob", 0.2)
    tension = params.get("tension", 0.2)
    rest_prob = params.get("rest_prob", 0.1)
    syncopation = params.get("syncopation", 0.0)
    dissonance = params.get("dissonance", 0.0)
    arpeggiation = params.get("arpeggiation", 0.0)
    ...
    for step in range(length):
        if use_motif and step % 8 >= motif_length and len(motif) >= motif_length:
            notes.append(motif[step % motif_length])
            continue
        ...
        if step % 2 == 1 and random.random() < syncopation:
            notes.append(notes[-1] if notes else scale[current_idx])
            continue

        if random.random() < arpeggiation:
            jump = random.choice([-2, 2, -4, 4])
        elif random.random() < leap_prob:
...
def _compose_bass_voice(root_midi: int, chord_prog: list, length: int, params: dict) -> list:
    ...
    for step in range(length):
        ...
        if groove > 0.4 and step % 2 == 1:
            if random.random() < groove:
                bass_note += random.choice([7, 12])
...
def compose_arrangement(emotion: str, root_midi: int = 60, length: int = 20, bpm: float = 120.0,
    ...
    if musical_params:
        ...
        direction_map = {"major": 0.5, "dorian": 0.1, "minor": -0.4, "phrygian": -0.7, "lydian": 0.4, "mixolydian": 0.3, "harmonic_minor": -0.6}
        ...
        params = {
            ...
            "rest_prob": rest_prob,
            "direction": direction, 
            "tension": max(0.05, 0.5 - note_density),
            "syncopation": musical_params.get("syncopation", 0.0),
            "arpeggiation": musical_params.get("arpeggiation", 0.0),
            "dissonance": musical_params.get("dissonance", 0.0),
            "groove": musical_params.get("groove", 0.0)
        }
    else:
        params = EMOTION_PARAMS.get(emotion, EMOTION_PARAMS["CALM"])
    scale = _build_scale(root_midi, params["mode"])
